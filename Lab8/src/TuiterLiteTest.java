...
//
    ...
    @Test
    public void testeCheckedExceptionUsuarioDesconhecido() throws TamanhoMaximoExcedidoException,
            UsuarioDesconhecidoException {
        Usuario impostor = new Usuario("impostor","impostor@gmail.com");

        try{
            tuiterLite.tuitarAlgo(impostor,"vou tentar invadir o sistema hehe");
            fail("o usuario não cadastrado");
        }
        catch(UsuarioDesconhecidoException e){
            assertFalse("O usuario não pode postar sem estar cadastrado",
                    this.tuiterLite.getUsuarioByEmail().containsValue(impostor));
        }
    }

    @Test
    public void testeCheckedExceptionTamanhoMaximoExecido() throws TamanhoMaximoExcedidoException,
            UsuarioDesconhecidoException {

        Usuario usuario = new Usuario("nome","email");
        tuiterLite.cadastrarUsuario(usuario.getNome(),usuario.getEmail());
        ...
        try{
            tuiterLite.tuitarAlgo(usuario,texto);
            fail("o tuite passou da quantidade de characters permitidos");
        }
        catch(TamanhoMaximoExcedidoException e){
            assertTrue(texto.length()>tuiterLite.TAMANHO_MAXIMO_TUITES);
        }
